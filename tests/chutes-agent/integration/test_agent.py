...
def test_api_models_integration():
    ...
    # Test request model
    request = StartMonitoringRequest(control_plane_url="https://control.example.com")
...
async def test_error_propagation(resource_monitor):
    ...
    with patch.object(
        ...
        side_effect=Exception("Client connection failed"),
    ):
        try:
...
async def test_watch_event_handling_integration(mock_watch, mock_client_class, resource_monitor):
