...
async def test_send_initial_resources_success(
    mock_sign, mock_aiohttp_session, control_plane_client, cluster_resources_with_objects
):
...
@pytest.mark.asyncio
@patch("chutes_agent.client.sign_request")
async def test_send_initial_resources_failure(
...
@pytest.mark.asyncio
@patch("chutes_agent.client.sign_request")
async def test_send_resource_update_success(mock_sign, mock_aiohttp_session, control_plane_client):
    """Test successful resource update send"""
    # Setup mocks
    mock_sign.return_value = ({"Authorization": "Bearer token"}, {"data": "payload"})

    event = WatchEvent(type=WatchEventType.ADDED, object=MagicMock())
...
@pytest.mark.asyncio
@patch("chutes_agent.client.sign_request")
async def test_register_cluster_payload_inspection(
    mock_sign, mock_aiohttp_session, control_plane_client, cluster_resources_with_objects
):
...
@pytest.mark.asyncio
@patch("chutes_agent.client.sign_request")
async def test_register_cluster_error_handling(
    mock_sign, mock_aiohttp_session, control_plane_client
):
