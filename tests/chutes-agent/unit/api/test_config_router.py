...
def setup(
    mock_core_client_class, mock_batch_client_class, mock_apps_client_class, mock_load_k8s_config
):
    pass
...
async def test_get_kubeconfig_from_secret_success(mock_core_client):
    ...
    mock_core_client.read_namespaced_secret.assert_called_once_with(
        name="miner-kubeconfig", namespace="default"
    )


@pytest.mark.asyncio
async def test_get_kubeconfig_from_secret_not_found(mock_core_client):
    """Test kubeconfig retrieval when secret not found"""
    from kubernetes_asyncio.client.rest import ApiException

    mock_core_client.read_namespaced_secret.side_effect = ApiException(
...
@pytest.mark.asyncio
async def test_get_kubeconfig_from_secret_permission_denied(mock_core_client):
...
async def test_get_kubeconfig_from_secret_custom_params(mock_core_client):
    ...
    result = await get_kubeconfig_from_secret(
        secret_name="custom-secret", namespace="custom-namespace"
    )
