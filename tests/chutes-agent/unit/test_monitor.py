...
def setup(
    mock_load_k8s_config, mock_core_client_class, mock_batch_client_class, mock_apps_client_class
):
    pass


def test_resource_monitor_init(mock_core_client, mock_batch_client, mock_apps_client):
...
async def test_initialize_success(
    ...
    mock_core_client_class,
    mock_apps_client_class,
    mock_batch_client_class,
    resource_monitor,
):
    """Test successful initialization"""
    # Setup mocks
    resource_monitor.control_plane_client = AsyncMock()
    resource_monitor.collector.collect_all_resources = AsyncMock(
        return_value={"pods": [], "deployments": [], "services": [], "nodes": []}
    )
...
async def test_handle_resource_event(resource_monitor):
    ...
    event = WatchEvent(type="ADDED", object=MagicMock())
...
async def test_handle_resource_event_error(resource_monitor):
    ...
    # Setup
    resource_monitor.control_plane_client = AsyncMock()
    resource_monitor.control_plane_client.send_resource_update.side_effect = Exception(
        "Network error"
    )
