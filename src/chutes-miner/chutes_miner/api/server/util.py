...
Kubeconfig and cache utilities, plus miner-API-specific server tracking
(track_server, start/stop_server_monitoring). For shared bootstrap and
verification, use chutes_miner.common.
"""
...
from chutes_common.monitoring.client import (
    ...
    stop_server_monitoring as _stop_server_monitoring_common,
)
...
async def start_server_monitoring(agent_url: str) -> None:
    """Start monitoring a server via the agent (miner API only)."""
...
async def stop_server_monitoring(agent_url: str) -> None:
    """Stop monitoring a server (miner API only)."""
    await _stop_server_monitoring_common(agent_url=agent_url)
...
async def track_server(
    ...
    """
    Track a new kubernetes (worker/GPU) node in our inventory (miner API only).
    Persists to Postgres. Registration API uses validator inventory instead.
    """
    ...
    cpu_count = int(node_object.status.capacity["cpu"]) - 2
