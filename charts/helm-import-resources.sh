...
process_chart() {
    ...
    TEMP_DIR=$(mktemp -d)
    helm template "$RELEASE_NAME" "$CHART_PATH" -f "$VALUES_FILE" -n "$NAMESPACE" --output-dir "$TEMP_DIR"
...
echo "helm upgrade --install $RELEASE_NAME1 $CHART_PATH1 -f $VALUES_FILE --namespace $NAMESPACE"
echo "helm upgrade --install $RELEASE_NAME2 $CHART_PATH2 -f $VALUES_FILE --namespace $NAMESPACE"